...
public class Wallet {
    ...
    public static WalletFile create(ChainTypes chainTypes, String password, IKeyPair keyPair, int n, int p)
        ...
        if (keyPair instanceof ECKeyPair) {
            privateKeyBytes = Numeric.toBytesPadded(keyPair.getPrivateKey(), Keys.PRIVATE_KEY_SIZE);
        } else {
            privateKeyBytes = keyPair.getSecret().getBytes(UTF_8);
        }
        ...
        return createWalletFile(chainTypes, keyPair, cipherText, iv, salt, mac, n, p);
    }

    public static WalletFile createStandard(ChainTypes chainTypes, String password, IKeyPair keyPair)
            throws CipherException {
        return create(chainTypes, password, keyPair, N_STANDARD, P_STANDARD);
    }

    public static WalletFile createLight(ChainTypes chainTypes, String password, IKeyPair keyPair)
            throws CipherException {
        return create(chainTypes, password, keyPair, N_LIGHT, P_LIGHT);
    }
    ...
    private static WalletFile createWalletFile(
        ...
        walletFile.setAddress(keyPair.getAddress());
        walletFile.setChainType(chainTypes.toString());
        walletFile.setChainType(chainTypes.toString());
        SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd HH-mm-ss");
        walletFile.setDateTime(format.format(System.currentTimeMillis()));
    ...
    public static IKeyPair decrypt(ChainTypes chainTypes, String password, boolean isED25519, WalletFile walletFile) throws CipherException {
